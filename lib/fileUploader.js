...
let uploadFile = (userId, fileKey) => {
  ...
  return new Promise ((resolve, reject) => {
    fs.readFile(filePath, function (error, data){
      ...
      let payload = {Bucket: FILES_BUCKET, Key: userId + '/' + fileKey, Body: data};
...
let uploadFileToBucket = (userId, fileKey, userImageUrl) => {
  return new Promise((resolve, reject) => {
    getFileByUrl(fileKey, userImageUrl).then((data) => {
        uploadFile(userId, fileKey).then((result) => {
          resolve(result)
        }).catch((e) => {
          reject({'error': e})
        });
